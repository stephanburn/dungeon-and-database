...
import { languageCreateSchema, languageDeleteSchema, languageUpdateSchema } from '@/lib/content/admin-schemas'
...
export async function POST(request: NextRequest) {
  ...
  const parsed = languageCreateSchema.safeParse(bodyResult.data)
  ...
  await writeAuditLog({
    actorUserId: user.id,
    action: 'content.language_created',
...
export async function PUT(request: NextRequest) {
  ...
  const bodyResult = await readJsonBody<Record<string, unknown>>(request)
  if ('response' in bodyResult) return bodyResult.response
  const parsed = languageUpdateSchema.safeParse(bodyResult.data)
  if (!parsed.success) return jsonError(parsed.error.message, 400)
  ...
  await writeAuditLog({
    actorUserId: user.id,
    action: 'content.language_updated',
    targetTable: 'languages',
    targetId: key,
    details: { key },
  })
...
export async function DELETE(request: NextRequest) {
  ...
  const parsed = languageDeleteSchema.safeParse({
    key: request.nextUrl.searchParams.get('key'),
  })
  if (!parsed.success) return jsonError(parsed.error.message, 400)

  const { error } = await supabase.from('languages').delete().eq('key', parsed.data.key)
  if (error) return jsonError(error.message, 500)
  await writeAuditLog({
    actorUserId: user.id,
    action: 'content.language_deleted',
    targetTable: 'languages',
