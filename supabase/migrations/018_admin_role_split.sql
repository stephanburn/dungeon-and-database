-- Split platform administration from campaign DM duties.
-- Admins keep all DM capabilities, but only admins can manage shared content and user roles.
...
-- The first signup becomes the initial admin instead of a DM.
...
-- Existing projects may already have a first DM but no admin yet.
DO $$
DECLARE
  first_user_id uuid;
BEGIN
