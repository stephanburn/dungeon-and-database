-- Slice 4.5c: preserve per-level provenance anchors. The original
-- sync_character_class_levels_for_class implementation deleted then reinserted
-- per-level rows, which could trigger ON DELETE SET NULL on choice-table
-- character_level_id columns. This version updates existing rows in place and
-- inserts only missing rows, so direct character_levels edits no longer strip
-- child-row provenance.
--
-- The level-up RPC carries forward Slice 4.5a/4.5b behavior and narrows
-- character_level_id fallback rules so preserved rows keep their original
-- anchor instead of being moved to the newly-created level.
...
BEGIN
  ...
  -- Only remove rows that no longer represent a valid level after an explicit
  -- class-level decrease. Rows that still correspond to 1..v_current.level are
  -- updated in place below so existing choice-table FKs remain intact.
  DELETE FROM public.character_class_levels
  WHERE character_id = p_character_id
    AND class_id = p_class_id
  ...
  SELECT
    COALESCE(existing.id, CASE WHEN series.level_number = v_current.level THEN v_current.id ELSE gen_random_uuid() END),
    p_character_id,
    p_class_id,
    series.level_number,
    CASE
      WHEN series.level_number = v_current.level THEN v_current.subclass_id
      ELSE existing.subclass_id
    END,
    COALESCE(hpr.roll, CASE WHEN series.level_number = v_current.level THEN v_current.hp_roll ELSE existing.hp_roll END),
    COALESCE(existing.taken_at, v_current.taken_at)
  ...
  SET
    ...
    taken_at = character_class_levels.taken_at;
END;
$$;

WITH latest_class_level AS (
  SELECT DISTINCT ON (character_id, class_id)
    id,
    character_id,
    class_id
  FROM public.character_class_levels
  ORDER BY character_id, class_id, level_number DESC
)
UPDATE public.character_spell_selections spell_choice
SET character_level_id = latest_class_level.id
FROM latest_class_level
WHERE spell_choice.character_level_id IS NULL
  AND spell_choice.owning_class_id IS NOT NULL
  AND latest_class_level.character_id = spell_choice.character_id
  AND latest_class_level.class_id = spell_choice.owning_class_id;
...
UPDATE public.character_language_choices language_choice
SET character_level_id = latest_class_level.id
FROM latest_class_level
WHERE language_choice.character_level_id IS NULL
  AND language_choice.source_category = 'class'
  AND latest_class_level.character_id = language_choice.character_id
  AND latest_class_level.class_id = language_choice.source_entity_id;
...
UPDATE public.character_tool_choices tool_choice
SET character_level_id = latest_class_level.id
FROM latest_class_level
WHERE tool_choice.character_level_id IS NULL
  AND tool_choice.source_category = 'class'
  AND latest_class_level.character_id = tool_choice.character_id
  AND latest_class_level.class_id = tool_choice.source_entity_id;
...
UPDATE public.character_ability_bonus_choices ability_choice
SET character_level_id = latest_class_level.id
FROM latest_class_level
WHERE ability_choice.character_level_id IS NULL
  AND ability_choice.source_category = 'class'
  AND latest_class_level.character_id = ability_choice.character_id
  AND latest_class_level.class_id = ability_choice.source_entity_id;
...
UPDATE public.character_feature_option_choices feature_choice
SET character_level_id = latest_class_level.id
FROM latest_class_level
WHERE feature_choice.character_level_id IS NULL
  AND feature_choice.source_category = 'class'
  AND latest_class_level.character_id = feature_choice.character_id
  AND latest_class_level.class_id = feature_choice.source_entity_id;
...
UPDATE public.character_skill_proficiencies skill_choice
SET character_level_id = latest_class_level.id
FROM latest_class_level
WHERE skill_choice.character_level_id IS NULL
  AND skill_choice.source_category = 'class'
  AND latest_class_level.character_id = skill_choice.character_id
  AND latest_class_level.class_id = skill_choice.source_entity_id;
...
BEGIN
  ...
  -- skill_proficiencies: the table's primary key is (character_id, skill), so
  -- a multiclass grant that overlaps an existing skill must not error. Merge
  -- expertise with OR semantics; preserve the earlier grant's provenance.
  IF p_payload ? 'skill_proficiencies' THEN
    ...
    DO UPDATE SET
      expertise = character_skill_proficiencies.expertise OR EXCLUDED.expertise;
  END IF;
  ...
  IF p_payload ? 'feature_option_choices' THEN
    ...
    SELECT
      ...
      COALESCE(incoming.character_level_id, existing.character_level_id, v_level_row_id),
    ...
    LEFT JOIN public.character_feature_option_choices existing
      ON existing.character_id = p_character_id
      AND existing.option_group_key = incoming.option_group_key
      AND existing.option_key = incoming.option_key
      AND existing.choice_order = COALESCE(incoming.choice_order, 0)
      AND existing.source_feature_key IS NOT DISTINCT FROM incoming.source_feature_key
  ...
  IF p_payload ? 'feat_choices' THEN
    ...
    LEFT JOIN public.character_feat_choices existing
      ON existing.character_id = p_character_id
      AND existing.feat_id = incoming.feat_id
      AND existing.choice_kind = COALESCE(incoming.choice_kind, 'feat')
