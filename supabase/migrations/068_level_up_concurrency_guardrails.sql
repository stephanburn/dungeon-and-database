-- Slice 4.5d: add concurrency guardrails on top of the Slice 4.5c provenance
-- fixes. Trigger-driven per-level sync now locks the owning character row, and
-- level-up saves reject stale expected_updated_at tokens before applying any
-- mutations.
--
...
BEGIN
  ...
  IF v_expected_updated_at IS NULL THEN
    RAISE EXCEPTION 'Optimistic lock token is required'
      USING ERRCODE = 'P0001';
  END IF;

  IF v_existing.updated_at IS DISTINCT FROM v_expected_updated_at THEN
    RAISE EXCEPTION 'Optimistic lock mismatch'
      USING ERRCODE = 'P0001';
  END IF;
