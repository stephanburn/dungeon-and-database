-- Slice 4.5b: make save_character_level_up_atomic tolerate skill overlap on
-- multiclass (and any other) skill grants. The target table has a narrow
-- primary key (character_id, skill); the RPC previously issued a straight
-- INSERT that raised a unique-key violation whenever a granted skill was
-- already held. Path B (recorded decision): keep the narrow PK, merge on
-- conflict, and rely on the existing single-row-per-skill read path.
--
-- Conflict policy:
--   * expertise = expertise OR EXCLUDED.expertise, so a later expertise
--     promotion (e.g. Knowledge Domain) never downgrades an existing row
--   * source_category / source_entity_id / source_feature_key / character_level_id
--     are preserved on conflict, i.e. first-write-wins for provenance. This
--     matches the sheet's one-row-per-skill display and avoids losing the
--     earlier grant's provenance when a later class re-asserts the same skill.
